...
export default function InternshipsForStudentsPage() {
  ...
  useEffect(() => {
    const fetchInternships = async () => {
      try {
        // Assuming a filter for 'students' might exist, or fetching all and filtering client-side
        // For now, fetching all internships
        const data = await getFilteredItems("internships", {});
        setInternships(data);
      } catch (err) {
        setError("Failed to fetch internships for students");
        console.error(err);
      } finally {
        setLoading(false);
      }
      ...
      <main className="bg-gray-50 min-h-screen py-12">
        <div className="container mx-auto px-6 max-w-4xl">
          <h1 className="text-3xl font-bold font-inter text-gray-900 mb-8 text-center flex items-center justify-center gap-3">
            <FaGraduationCap className="text-gray-700" /> Internships for Students
          </h1>
          ...
          {internships.length > 0 ? (
            ...
            <p className="text-gray-700 text-lg text-center">No internships found for students at the moment.</p>
          )}
        </div>
      </main>
      <Footer />
