...
export default function JobsLocationsPage(){
  ...
  useEffect(() => {
    const fetchLocations = async () => {
      try {
        const data = await getListFilterOptions("jobs", "job_location");
        setLocations(data);
      } catch (err) {
        setErrorLocations("Failed to fetch job locations");
        console.error(err);
      } finally {
    ...
    return (
      <>
        ...
        <main className="bg-gray-50 min-h-screen py-12 flex items-center justify-center">
          <p className="text-gray-700 text-lg">Loading job locations...</p>
        </main>
        <Footer />
      </>
    );
      ...
      <main className="bg-gray-50 min-h-screen py-12">
        <div className="container mx-auto px-6">
          <h1 className="text-3xl font-bold font-inter text-gray-900 mb-8 text-center flex items-center justify-center gap-3">
            <FaMapMarkerAlt className="text-gray-700" /> Browse Job Locations
          </h1>
          ...
          {/* Jobs List */}
          <h2 className="text-2xl font-bold font-inter text-gray-900 mb-6">Available Jobs {selectedLocation ? `in ${selectedLocation}` : ""}</h2>
          {loadingJobs ? (
            ...
            <p className="text-gray-700 text-lg text-center">No jobs found {selectedLocation ? `for ${selectedLocation}` : ""}.</p>
          )}
        </div>
      </main>
      <Footer />
