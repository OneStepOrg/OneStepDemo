...
export default function InternshipsWithStipendPage() {
  ...
  useEffect(() => {
    const fetchInternships = async () => {
      try {
        // Assuming 'paid_unpaid' filter exists in the API for internships
        const data = await getFilteredItems("internships", { paid_unpaid: "Paid" });
        setInternships(data);
      } catch (err) {
        setError("Failed to fetch internships with stipend");
        console.error(err);
      } finally {
        setLoading(false);
      }
      ...
      <main className="bg-gray-50 min-h-screen py-12">
        <div className="container mx-auto px-6 max-w-4xl">
          <h1 className="text-3xl font-bold font-inter text-gray-900 mb-8 text-center flex items-center justify-center gap-3">
            <FaMoneyBillWave className="text-gray-700" /> Internships with Stipend
          </h1>
          ...
          {internships.length > 0 ? (
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {internships.map((internship) => (
                <div key={internship.id} className="bg-white p-6 rounded-lg shadow-md border border-gray-200">
                  ...
                  <p className="text-gray-600 mb-1"><strong>Work Mode:</strong> {internship.work_mode}</p>
                  <p className="text-gray-600"><strong>Stipend:</strong> ₹{internship.stipend}/month</p>
            ...
            <p className="text-gray-700 text-lg text-center">No internships with stipend found at the moment.</p>
          )}
        </div>
      </main>
      <Footer />
